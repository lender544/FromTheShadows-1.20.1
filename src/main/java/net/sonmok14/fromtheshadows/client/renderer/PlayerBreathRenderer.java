...
@OnlyIn(Dist.CLIENT)
public class PlayerBreathRenderer extends EntityRenderer<PlayerBreathEntity> {
    private static final ResourceLocation TEXTURE = new ResourceLocation(Fromtheshadows.MODID, "textures/entity/player_breath.png");
    ...
    public PlayerBreathRenderer(EntityRendererProvider.Context mgr) {
        super(mgr);
    }

    @Override
    public ResourceLocation getTextureLocation(PlayerBreathEntity entity) {
        return TEXTURE;
    }

    @Override
    public void render(PlayerBreathEntity playerBreath, float entityYaw, float delta, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
        clearerView = playerBreath.caster instanceof Player && Minecraft.getInstance().player == playerBreath.caster && Minecraft.getInstance().options.getCameraType() == CameraType.FIRST_PERSON;

        double collidePosX = playerBreath.prevCollidePosX + (playerBreath.collidePosX - playerBreath.prevCollidePosX) * delta;
        double collidePosY = playerBreath.prevCollidePosY + (playerBreath.collidePosY - playerBreath.prevCollidePosY) * delta;
        double collidePosZ = playerBreath.prevCollidePosZ + (playerBreath.collidePosZ - playerBreath.prevCollidePosZ) * delta;
        double posX = playerBreath.xo + (playerBreath.getX() - playerBreath.xo) * delta;
        double posY = playerBreath.yo + (playerBreath.getY() - playerBreath.yo) * delta;
        double posZ = playerBreath.zo + (playerBreath.getZ() - playerBreath.zo) * delta;
        ...
        int frame = Mth.floor((playerBreath.appear.getTimer() - 1 + delta) * 2);
        if (frame < 0) {
            frame = 6;
        }
        VertexConsumer ivertexbuilder = bufferIn.getBuffer(FTSRenderType.getGlowingEffect(getTextureLocation(playerBreath)));
        ...
        renderEnd(frame, playerBreath.blockSide, matrixStackIn, ivertexbuilder, packedLightIn);
